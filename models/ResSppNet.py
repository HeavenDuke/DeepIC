...
def EnhancedResSppNet(class_num, enhanced_class_num):
    ...
    model1.compile(loss = "categorical_crossentropy", optimizer = RMSprop(lr = 1e-3, decay = 1e-4), metrics = ['accuracy'])

    model2 = Dense(units = enhanced_class_num)(model)
    model2 = Activation(activation = "softmax")(model2)
    model2 = Model(_input, model2)
    model2.compile(loss = "categorical_crossentropy", optimizer = RMSprop(lr = 1e-3, decay = 1e-4), metrics = ['accuracy'])
